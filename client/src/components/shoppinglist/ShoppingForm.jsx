...
const ShoppingForm = (props) => {
  ...
  useEffect(() => {
    if (props.showModify) {
      ...
      setDate(props.showModify.buy_by_date ? props.showModify.buy_by_date : "");
  ...
  const handleSubmit = () => {
    ...
    if (!(ingredient_name == null || ingredient_name == "") && !(quantity == null || quantity == "") && !(dayjs(date).isBefore(dayjs(), 'day'))) {
      ...
      if (props.showModify && props.p == false) {
        ...
        API.updateItemShoppingList(props.showModify.id, item).then(() => {
    ...
    <>
      ...
      <Modal show={props.show} dialogClassName="modal-dialog-bottom modal-open" onHide={() => { animationOut(); }} animation={false}
        ...
        <Modal.Header
          ...
          {props.p ? <Modal.Title>Add to Shopping List</Modal.Title> : <Modal.Title>Update Item</Modal.Title>}
        </Modal.Header>
        <Modal.Body style={{ minHeight: 300 }}>
          ...
          <Form>
            ...
            <Row>
              <Col xs={6}>
                <Form.Label>Buy by date:</Form.Label>
                <Form.Control
                  type="date"
                  value={date}
